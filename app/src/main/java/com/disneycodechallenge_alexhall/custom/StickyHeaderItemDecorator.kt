package com.disneycodechallenge_alexhall.custom
...
class StickyHeaderItemDecorator(private val adapter: StickyAdapter<*, *>) : ItemDecoration() {
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (preOverlappedPosition != overlappedHeaderPosition && viewOverlappedByHeader.shouldMoveHeader()
        ) {
    ...
    private fun View?.shouldMoveHeader(): Boolean {
        val dy = this!!.top - height
        return top >= 0 && dy <= 0
    }
